...
return;
  ...
  if (users.cookies < rob) return conn.reply(m.chat, `😔 @${who.split`@`[0]} 𝒕𝒊𝒆𝒏𝒆 𝒎𝒆𝒏𝒐𝒔 𝒅𝒆 *${ro} Dragones 🌀*\n𝑵𝒐 𝒓𝒐𝒃𝒆𝒔 𝒂 𝒖𝒏 𝒑𝒐𝒃𝒓𝒆 🥺`, m, {mentions: [who]});
  global.db.data.users[m.sender].cookies += rob;
  global.db.data.users[who].cookies -= rob;
  conn.reply(m.chat, `*🔥 𝑹𝒐𝒃𝒂𝒔𝒕𝒆 ${rob} Dragones 🌀 𝒂 @${who.split`@`[0]}*`, m, {mentions: [who]});
